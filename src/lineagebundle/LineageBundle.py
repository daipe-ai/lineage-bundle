...
class LineageBundle(Bundle):
    ...
    def __get_mapping(self):
        decorator_mapping_entry_points = entry_points_reader.get_by_key("daipe.decorator_mapping")

        all_mapping = dict()

        for decorator_mapping_entry_point in decorator_mapping_entry_points:
            decorator_mapping: DecoratorMappingInterface = attribute_loader.load_from_string(decorator_mapping_entry_point.value)()
            all_mapping = {**all_mapping, **decorator_mapping.get_mapping()}
